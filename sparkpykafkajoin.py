...
spark = SparkSession.builder.appName('redis-console-log').getOrCreate()
...
redisStreamingDF = redisRawStreamingDF\
...
zSetEntriesEncodedStreamingDF = spark\
    .sql('select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet')
...
emailAndBirthDayStreamingDF = spark\
    .sql('select customerName, email, phone, birthDay from CustomerRecords where (email is not null) and (birthDay is not null)')
...
emailAndBirthYear = emailAndBirthDayStreamingDF\
...
riskScoreByBirthYear = emailAndBirthYear.join(emailAndRiskScore, expr("""
    email = customer
"""))
